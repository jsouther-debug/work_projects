...
List asList() => [this.x, this.y, this.z];
...
static Point average(Vector a, Vector b) => Point(
(a.x + b.x) / 2,
(a.y + b.y) / 2,
(a.z + b.z) / 2,
);

static Point lerp(Vector a, Vector b, double factor) => Point(
...
Direction.fromPoints(Vector from, Vector to) : this.fromVector(to - from);
...
Normal.fromPoints(Vector from, Vector to) : this.fromVector(to - from);
...
Ray.fromPoints(Vector from, Vector to) {
...
Frame({Vector o, Direction x, Direction y, Direction z, Normal n}) {
...
Frame.lookAt(Vector eye, Vector target, Direction up) {
o = eye;
...
Point w2lPoint(Vector p) {
Vector d = p - o;
return Point(x.dot(d), y.dot(d), z.dot(d));
}
Point l2wPoint(Vector p) {
