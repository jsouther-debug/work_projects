...
class Vector2 {
  ...
  Vector2.fromPolar(double radians, num radius)
      : this(cos(radians) * radius, sin(radians) * radius);
...
class Point2 extends Vector2 {
  ...
  Point2.fromPolar(double radians, num radius)
      : this(cos(radians) * radius, sin(radians) * radius);

  String toString() => 'Point2($x, $y)';

  static Point2 average(Point2 a, Point2 b) =>
...
class Point2i extends Vector2i {
  ...
  Point2i.fromPolar(double radians, num radius)
      : this(cos(radians) * radius, sin(radians) * radius);
...
class Vector2i {
  ...
  Vector2i.fromPolar(double radians, num radius)
      : this(cos(radians) * radius, sin(radians) * radius);
...
class Point extends Vector {
  ...
  Point.fromList(List p)
      : super(p[0].toDouble(), p[1].toDouble(), p[2].toDouble());
...
class Direction extends Vector {
  ...
  Direction.fromList(List p)
      : this(p[0].toDouble(), p[1].toDouble(), p[2].toDouble());
...
class Color {
  ...
  int redAsInt({int maxv = 255}) =>
      max(0, min(maxv, (red * (maxv + 0.999)).toInt()));
  int greenAsInt({int maxv = 255}) =>
      max(0, min(maxv, (green * (maxv + 0.999)).toInt()));
  int blueAsInt({int maxv = 255}) =>
      max(0, min(maxv, (blue * (maxv + 0.999)).toInt()));
  int alphaAsInt({int maxv = 255}) =>
      max(0, min(maxv, (alpha * (maxv + 0.999)).toInt()));
  ...
  double get lightness =>
      0.5 * (max(max(red, green), blue) + min(min(red, green), blue));
...
class Frame {
  ...
  Frame.fromJson(JsonLoader loader)
      : this(
