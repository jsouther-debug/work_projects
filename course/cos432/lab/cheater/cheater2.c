...
#define BREAK_POINT_ADDRESS     0x55555555E220 //right after it gets an input from the user
#define BREAK_POINT_ADDRESS_2   0x55555555B3B2 // breakpoint to change coords
...
uint64_t set_register(pid_t pid, int reg, uint64_t new_value) {
    ...
    switch (reg) {
        ...
        case REG_R10:
            former_value = regs.r10;
            regs.r10 = new_value;
            break;

        case REG_R13:
            former_value = regs.r13;
            regs.r13 = new_value;
            break;
...
void wait_for_inferior(pid_t pid) {
    ...
    int ready;
    int status;
    uint64_t save = 0, save2 = 0, ip;
    int first_stop = 1;
    uint32_t save32reg = 0;
    uint64_t save64reg1 = 0;
    uint64_t save64reg2 = 0;
        ...
        if (WIFEXITED(status)) {
            ...
            if (WSTOPSIG(status) == SIGALRM) {
                ...
                else
                {
                    ...
                    if (ip - 1 == BREAK_POINT_ADDRESS && ready != 1) {
                        unpoke_int3(pid, BREAK_POINT_ADDRESS, save);
                        //printf("First BP, ready flag set.\n");
                        ready = 1;
                        ...
                        save2 = poke_int3(pid, BREAK_POINT_ADDRESS_2);
                    }
                    ...
                    if (ready == 1) {
                        ...
                        if (ip - 1 == BREAK_POINT_ADDRESS_2)
                        {
                            printf("Inside second breakpoint.\n");
                            unpoke_int3(pid, BREAK_POINT_ADDRESS_2, save2);

                            set_register(pid, REG_RIP, BREAK_POINT_ADDRESS_2);

                            uint64_t reg10 = get_register(pid, REG_R10);
                            uint64_t reg13 = get_register(pid, REG_R13);
                            ...
                            //set_register(pid, REG_R10, 0x11);
                            set_register(pid, REG_R13, 0x5);

                            //reg10 = get_register(pid, REG_R10);
                            //reg13 = get_register(pid, REG_R13);
